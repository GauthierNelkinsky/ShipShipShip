...
<div transition:fly={{ y: 20, duration: 300, easing: quintOut }}>
    <div
        class="p-4 cursor-pointer transition-all duration-200 hover:shadow-md hover:scale-[1.02] transform {isDragging ||
        isBeingDragged
            ? 'opacity-50 rotate-2 scale-95 ring-2 ring-primary'
            : ''} group bg-card text-card-foreground rounded-lg border border-border shadow-sm"
        draggable="true"
    ...
    >
        ...
        <div class="flex items-start justify-between mb-3">
            <h3 class="font-semibold text-sm leading-tight flex-1 pr-2">
                {event.title}
            </h3>
            <div
        ...
        <!-- Content Preview -->
        {#if event.content}
            <div class="text-xs text-muted-foreground mb-3 line-clamp-3">
                {@html markdownToHtml(
